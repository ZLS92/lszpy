...
def load_oasis_montaj_grid(fname):
    ...
    grid = np.rot90( np.fliplr( grid.reshape(shape, order=order)), 2 )
    # Build coords
...
def _get_data_type(n_bytes_per_element, sign_flag):
    ...
    if n_bytes_per_element == 1:
        ...
        if sign_flag == 0:
            Gdata_type = GDALDataType["GDT_UInt32"]  # unsigned int
...
def grid2xyz( fname, out_fmt='points' ) :

    grid,header,_ = load_oasis_montaj_grid( fname )
    ...
    xstart, xstop = header['x_origin'], header['x_origin']+nx*dx
    ystart, ystop = header['y_origin'], header['y_origin']+ny*dy
    x = np.linspace(xstart, xstop, nx)
    y = np.linspace(ystop, ystart, ny)
    xx, yy = np.meshgrid( x, y, copy=False )
    zz = grid

    if out_fmt == 'points':
        xx = xx[~np.isnan(grid)].ravel()
        yy = yy[~np.isnan(grid)].ravel()
        zz = grid[~np.isnan(grid)].ravel()
