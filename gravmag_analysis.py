...
def parker( array, rho, n=5, sx=1.0, sy=1.0, padw=0, pmode='gdal', nanfill='gdal',
    ...
    # shift_array =  array - np.nanmean( array )
    ar_pad, original_shape_indx = utl.pad_array( array, padw, pmode )
    # ar_pad = ar_pad + np.nanmean( array )
...
def polyfit2d(array, order=[1], mask=None, sx=1.0, sy=1.0, lim=None, 
    ...
    if plot == True:
        ...
        utl.plta( pp, sbplt=[1, 3, 2], tit='Trend' )
        utl.plta( rr, sbplt=[1, 3, 3], tit='Residual', vmin=vmin, vmax=vmax )
...
# -----------------------------------------------------------------------------
def radial_fft( array, sx=1, sy=1, plot=False, units='[1/units]', title=False, remove='mean',
                padw=0, pmode='symmetric', fillnan=None, order=[1], alpha=None,
                plot2D=True, maxSearchDist=None, iter=None, smooth=0, tension=0.35 ) :
    ...
    ar_pad, original_shape_indx = utl.pad_array( array, padw, pmode, alpha=alpha, sqr_area=False )
    ...
    if plot == True:
        ...
        if plot2D == True :
            plt.subplot(2, 2, 1)
            plt.pcolormesh( ( np.log(SPDS) ), cmap='jet', 
                vmin=np.log(PDS_radial).min(), vmax=np.log(PDS_radial).max())
            ...
            if title != False:
                plt.title(title)
            plt.axis('tight')
            plta( ar_pad, sbplt=[2, 2, 2])
            i += 1

        ax1 = plt.subplot(1+i, 1, 1+i)
        
        x = k_radial 
        ax1.plot( x, np.log(PDS_radial), '-' )   
        ax1.set_xlabel('Wavenumber ' + units)
        plt.ylabel('ln (PSD)')
        plt.tight_layout()
...
def DeTrend_filt( array, radius=4, pmode='linear_ramp', padw=0, alpha=None,
    ...
    @jit( parallel=True )
    def loop_par ( array, trend, ar_filt, radius, original_shape_indx, ar_pad ) :
        ...
        for idx, ai in np.ndenumerate( array ) :
            ...
            A = np.vstack([X1.ravel(), Y1.ravel(), np.ones(Y1.ravel().shape[0])]).T
            C,_,_,_ = utl.sp.linalg.lstsq( A, wi.ravel() )    # coefficients
    ...
    ar_filt, trend = loop_par(array, trend, ar_filt, radius, original_shape_indx, ar_pad )    
    ...
    if plot == True:
        ...
        plta( trend, vmin, vmax, sbplt=[1, 3, 2], tit='Trend')
        plta( ar_filt, vmin, vmax, sbplt=[1, 3, 3], tit='Filtered')
        plt.tight_layout() 
...
def terracing( array, padw=0, pmode='linear_ramp', remove='mean', order=[1], 
               alpha=0, mask=None, nodata=True, plot=False, vmin=None, vmax=None, 
               filt=False, ftype='hanning', iter=1, radius=1, iter_filt=1,
               pfilt=True,  psigma=1, ptruncate=4,
    ...
    xu = np.arange( 0, terr_pad.shape[1] )
    yu = np.arange( 0, terr_pad.shape[0] )
    ...
    for i in range( iter ) :
        ...
        if pfilt is True :
            terr_pad = utl.sp.ndimage.gaussian_filter( terr_pad, sigma=psigma, truncate=ptruncate )

        pad_lap = utl.sp.ndimage.laplace( terr_pad, mode='nearest' ) 
        pad_min = utl.sp.ndimage.minimum_filter( terr_pad, radius*2+1, mode='nearest' )
        pad_max = utl.sp.ndimage.maximum_filter( terr_pad, radius*2+1, mode='nearest' )
        pad_mean = utl.sp.ndimage.uniform_filter( terr_pad, radius*2+1, mode='nearest' )
        pad_var = pad_max - pad_min 
        idx_var = (pad_mean > threshold) | (pad_mean < - threshold)             
        idx_min = pad_lap < 0  
        idx_max = pad_lap > 0 
    ...
    if filt is True :
        terr = utl.filt2d( terr, ftype=ftype, iter=iter_filt, radius=radius )        
